    ...
    const fetchReading = async () => {
      try {
        const res = await fetch("/api/tarot", {
          ...
          body: JSON.stringify({ cards: cardIndexes, language, category }),
        });
      ...
      <div className="w-full max-w-4xl mx-auto z-10">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-br from-yellow-200 via-yellow-400 to-yellow-600 tracking-tight mb-4 drop-shadow-sm">
            {language === "ko" 
              ? (category === "love" ? "당신의 연애운" : category === "wealth" ? "당신의 금전운" : "당신의 운명") 
              : (category === "love" ? "Your Love Destiny" : category === "wealth" ? "Your Wealth Destiny" : "Your Destiny")}
          </h1>
        </div>

        {/* Shareable Destiny Card Section */}
        <CaptureWrapper filename="mystic-tarot">
          <div className="bg-slate-900/60 backdrop-blur-xl border border-indigo-500/30 rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden">
